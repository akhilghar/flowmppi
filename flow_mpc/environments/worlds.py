...
class World:
    ...
    def __init__(self, dim_world, world_size=4, gen_halton=True):
...
class FromFileWorld(World):
    ...
    def __init__(self, world_dim, world_size, filename):
        super().__init__(dim_world=world_dim, world_size=world_size, gen_halton=False)
        data_dict = dict(np.load(f'{FLOW_MPC_ROOT}/../worlds/{filename}'))
        self.grid = data_dict['arr_0']
        self.grid = np.swapaxes(self.grid, axis1=1, axis2=2)

        #self.grid = cv2.resize(self.grid[0:50], (64,64,64))
...
class NarrowPassagesWorld(World):
    ...
    def _get_occupancy_grid(self):
        ...
        if self.dw == 2:
            ...
            # if 8 rooms
            if rooms == 8:
                # put doors i x-y plane
                grid = self._put_four_doors_in_wall(grid, self.door_width, self.door_width)
                # put doors in x-z plane
                grid = self._put_four_doors_in_wall(grid.transpose(0, 2, 1), self.door_width,
                                                    self.door_width).transpose(0,
                                                                               2,
                                                                               1)
                # put doors in y-z plane
                grid = self._put_four_doors_in_wall(grid.transpose(1, 2, 0), self.door_width,
                                                    self.door_width).transpose(2,
                                                                               0,
                                                                               1)
            else:
                # Doors in z direction
                grid = self._put_two_doors_in_wall(grid, self.door_width)
                grid = self._put_two_doors_in_wall(grid.transpose(1, 0, 2), self.door_width).transpose(1, 0, 2)

        return grid

    def _put_two_doors_in_wall(self, wall, door_width):
        # First door is in half 0:30
        door_low = np.random.randint(low=12, high=28 - door_width)
        wall[door_low:door_low + door_width] = 0.0

        # First door is in half 34:64
        door_low = np.random.randint(low=36, high=52 - door_width)
        wall[door_low:door_low + door_width] = 0.0
        return wall
    ...
    def _put_four_doors_in_wall(self, wall, door_width, door_height):
        ...
        door_x_low = np.random.randint(low=8, high=28 - door_width)
        door_y_low = np.random.randint(low=32, high=52 - door_height)
        wall[door_x_low:door_x_low + door_width, door_y_low:door_y_low + door_height] = 0.0

        # First door is in quadrant 32:64, 0:32
        door_x_low = np.random.randint(low=32, high=52 - door_width)
