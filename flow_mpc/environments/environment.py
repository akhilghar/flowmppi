...
class Environment:
    ...
    def __init__(self, state_dim, control_dim, world_dim, world_type):
        ...
        elif 'stairway' in world_type:
            self.world = FromFileWorld(world_dim=world_dim, world_size=4, filename='stairway.npz')
        elif 'rooms' in world_type:
            self.world = FromFileWorld(world_dim=world_dim, world_size=4, filename=f'{world_type}.npz')
        else:
    ...
    def reset(self, zero_velocity=False):
        self.world.reset()
        self.reset_start_and_goal(zero_velocity)
