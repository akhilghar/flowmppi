...
def add_trajectory_to_axis(ax, start, goal, trajectories, sdf):
    ...
    big_sdf = np.where(big_sdf < 0, 0, 1)
    ...
    for i in range(len(positions_idx)):
        ax.plot(positions_idx[i, :, 0], 255 - positions_idx[i, :, 1], linewidth=3)
    ax.plot(goal[0], 255 - goal[1], marker='o', color="red", markersize=5)
    ax.plot(start[0], 255 - start[1], marker='o', color="blue", markersize=5)
...
def plot_trajectories(planning_network, generative_model, starts, goals, sdf, name, config):
    # Send to device
...
def plot_sdf_samples(planning_network, normalised_sdf, name, config):
    # Send to device
    normalised_sdf = normalised_sdf.to(device=config['device'])
...
def plot_sdf_reconstructions(planning_network, normalised_sdf, name, config):
    # Send to device
    normalised_sdf = normalised_sdf.to(device=config['device'])
    d, w, h = normalised_sdf.shape[-3:]
    if d > 1:
